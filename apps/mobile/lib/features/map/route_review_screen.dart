...
class _RouteReviewScreenState extends State<RouteReviewScreen> {
  ...
  /// The route's stored marker review, carried through recalculation unchanged.
  ///
  /// Nothing edits it now that the junction-marker surfaces are gone; it is kept
  /// so a route saved by an earlier build round-trips rather than silently
  /// losing data.
  late final MarkerPlanReview _markerReview = widget.route.markerReview;
  ...
  // `_loadPointsOfInterest`, `_showPointOfInterest` and
  // `_pointOfInterestForPin` were here (#20). They read a 141 KB catalogue of
  // motorcycle cafes from ukbikercafes.co.uk and drew them as orange pins a
  // sailor could tap to add as a waypoint.
  //
  // The marine equivalent - harbours, marinas, anchorages, facilities - is #13,
  // and needs attributable licensed sources rather than a renamed cafe list.
              ...
              child: ListView(
                ...
                children: [
                  ...
                  if (widget.onReshapeRoute != null) ...[
                    ...
                    // The drawing hint no longer mentions orange places to
                    // tap (#20), and is shown whenever the passage can be
                    // reshaped rather than only for a destination plan.
                    if (widget.onReshapeRoute != null) ...[
                      const SizedBox(height: 6),
                      const Text(
                        'Drag the blue line to shape it. The preview '
                        're-plans before you confirm.',
                        style: TextStyle(
