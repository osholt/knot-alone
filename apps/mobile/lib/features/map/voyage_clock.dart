...
/// front of the sailor, so the clock belongs on the map. Apple's own widgets are
...
class VoyageClock extends StatefulWidget {
  const VoyageClock({super.key, this.clock, this.style, this.darkMap = true});
  ...
  @override
  State<VoyageClock> createState() => _VoyageClockState();
}

class _VoyageClockState extends State<VoyageClock> {
  Timer? _tick;
  ...
  Widget build(BuildContext context) {
    // The platform's own formatting, so a sailor who keeps their phone on a
...
/// which is also the longest a clock can be wrong by without a sailor noticing.
