...
class ObserverAccessSheet extends StatefulWidget {
  ...
  static Future<void> show(
    ...
    // drags, so the handle is the only dismissing gesture there is - and a sailor
...
class _ObserverAccessSheetState extends State<ObserverAccessSheet> {
  ...
  ObserverAccessScope _scope = ObserverAccessScope.sailor;
  bool _consent = false;
      ...
      builder: (context, _) => SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // target; a sailor who could not leave it was left stuck in the app
            ...
            const Text(
              ...
              'It shares your last-known position, update freshness, voyage '
              'status and your help or emergency-stop status. The watcher '
              'does not join the voyage or appear in the sailor list.',
            ),
            const SizedBox(height: 8),
            Text(
              _scope == ObserverAccessScope.group
                  ? 'It never shares the voyage code, location trails, nearby '
                        'identifiers, phone numbers, emergency-contact details '
                        'or participant controls. A missing update is not proof '
                        'that a sailor is safe.'
                  : 'It does not share the voyage code, other sailors, your '
            ...
            if (widget.canShareGroup) ...[
              ...
              SegmentedButton<ObserverAccessScope>(
                ...
                segments: const [
                  ButtonSegment(
                    value: ObserverAccessScope.sailor,
              ...
              Text(
                _scope == ObserverAccessScope.group
                    ? 'The watcher will see the current sailor list, each '
            ...
            if (!widget.controller.busy && widget.controller.grants.isEmpty)
              const Text('No safety links have been created for this voyage.'),
    ...
    await SharePlus.instance.share(
      ShareParams(
        text: invite.grant.scope == ObserverAccessScope.group
            ? 'Watch our group voyage using this private, read-only, '
                  'time-limited Tide and Seek link:\n${invite.shareUri}'
            : 'Follow my last-known voyage progress using this private, '
                  'time-limited Tide and Seek link:\n${invite.shareUri}',
