...
import 'voyage_recap_card.dart';

/// Shows [VoyageRecapCard] full-screen and shares it as a PNG - a purpose-made
/// image for social media, separate from the text/CSV/GPX voyage summary
/// share so it isn't diluted by data files receiving apps don't expect.
class VoyageRecapScreen extends StatefulWidget {
  const VoyageRecapScreen({
    super.key,
  ...
  @override
  State<VoyageRecapScreen> createState() => _VoyageRecapScreenState();
}

class _VoyageRecapScreenState extends State<VoyageRecapScreen> {
  final _boundaryKey = GlobalKey();
  bool _sharing = false;
  String? _error;

  /// Simplified once, here, rather than per build or per caller: a two-hour
  /// voyage arrives with around ten thousand recorded points and the card is
  ...
  @override
  void didUpdateWidget(VoyageRecapScreen oldWidget) {
  ...
  /// a sailor stops waiting and gets an outline they can actually send.
  ...
  /// first Share does what a sailor expects, and stored nowhere.
  bool? _darkOverride;
  ...
  Future<void> _share() async {
    ...
    try {
      ...
      final fileName =
          'tide-and-seek-${widget.summary.voyageCode.toLowerCase()}-recap.png';
      await SharePlus.instance.share(
        ShareParams(
          text: 'Tide and Seek voyage recap · ${widget.summary.voyageCode}',
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('Voyage recap')),
    body: SafeArea(
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            SegmentedButton<bool>(
              ...
              selected: {_dark},
              // The image only. Nothing here touches the app theme, the voyage
