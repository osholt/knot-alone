...
/// Shows the voyage's invitation as a QR code, generated on this phone.
///
/// The point is not saving typing. Every other join path resolves through the
/// relay, so a group with no signal cannot form a voyage at all; a QR carries the
...
///
/// Anyone who photographs this can join the voyage, because the payload carries the
...
class VoyageInvitationQrSheet extends StatelessWidget {
  const VoyageInvitationQrSheet({super.key, required this.session});

  final VoyageSession session;

  static Future<void> show(BuildContext context, VoyageSession session) =>
      showModalBottomSheet<void>(
        context: context,
        showDragHandle: true,
        useSafeArea: true,
        isScrollControlled: true,
        builder: (_) => VoyageInvitationQrSheet(session: session),
      );

  @override
  Widget build(BuildContext context) {
    final payload = VoyageJoinPayload(
      ...
      child: Column(
        ...
        children: [
          ...
          Center(
            ...
            child: Container(
              key: const Key('voyage-invitation-qr'),
          ...
          Container(
            ...
            child: const Text(
              'Anyone who photographs this code can join the voyage. Show it to '
              'the sailors you meant to, then close this.',
