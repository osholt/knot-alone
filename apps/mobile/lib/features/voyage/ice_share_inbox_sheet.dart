...
/// Shows ICE (in-case-of-emergency) info other sailors have shared into this
/// voyage, and the read-receipt status of any the local sailor has sent.
...
/// used, which is what exempts it from the voyage-end purge.
class IceShareInboxSheet extends StatefulWidget {
  const IceShareInboxSheet({super.key, required this.voyageController});
  ...
  static Future<void> show(
    ...
    useSafeArea: true,
    showDragHandle: true,
    builder: (_) => IceShareInboxSheet(voyageController: voyageController),
  );
...
class _IceShareInboxSheetState extends State<IceShareInboxSheet> {
  ...
  void initState() {
    super.initState();
    for (final share in widget.voyageController.receivedIceShares) {
      unawaited(widget.voyageController.markIceInfoViewed(share.eventId));
    }
  }

  @override
  Widget build(BuildContext context) => AnimatedBuilder(
    animation: widget.voyageController,
    builder: (context, _) {
      final received = widget.voyageController.receivedIceShares;
      final sent = widget.voyageController.sentIceShares;
      return SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Only for this voyage - a skipper-received contact clears itself '
              'once the voyage ends, unless you called or texted it.',
            ...
            if (received.isEmpty)
              ...
              for (final share in received) ...[
                _ReceivedIceShareCard(
                  share: share,
                  onUsed: () =>
                      widget.voyageController.markIceShareUsed(share.eventId),
                ),
    ...
    child: Column(
      ...
      children: [
        Text(
          ...
          '${share.toWholeGroup ? '' : ' (shared with the skipper)'}',
