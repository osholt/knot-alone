...
import 'skipper_voyage_status.dart' show SweeperAvailability;
...
///
/// A skipper reading "1.2 miles" learns almost nothing - on a fast road that is
...
/// the second reading tells a skipper whether to slow down (#181).
enum SweeperGapTrend {
  ...
  /// stopped sailor, which is the distinction #132 and #134 both turned on.
  unknown,

  /// The TEC is not moving. A claim about the sailor, not about the gap.
  stopped,
  ...
  /// one that actually tells a skipper to ease off.
  opening,
}

extension SweeperGapTrendLabel on SweeperGapTrend {
  /// Paired with colour, never replaced by it. Sailors wear tinted visors in
  /// direct sunlight - the condition #107 and #143 exist for - and some sailors
  ...
  String get arrow => switch (this) {
    ...
    SweeperGapTrend.closing => '↓',
    SweeperGapTrend.holding => '=',
...
class _GapSample {
  const _GapSample({
    ...
    required this.sweeperPosition,
  });

  final DateTime at;
  final double gapMeters;
  final GeoPoint? sweeperPosition;
}
...
class SweeperGapTrendTracker {
  ...
  /// state, short enough that a genuine split shows up while the skipper can
  ...
  ///
  /// 0.6 m/s is about 2 km/h of difference between two sailors - below that they
  ...
  /// previous sailor's gap says nothing about the new one's.
  ...
  ///
  /// [availability] is the one TEC model from `skipper_voyage_status.dart`. Anything
  /// other than tracking clears the history and reports [SweeperGapTrend.unknown]:
  /// a gap computed from a fix that can no longer be trusted is not a gap, and a
  /// TEC whose position has gone stale is emphatically not a stopped sailor.
  SweeperGapTrend update({
    required SweeperAvailability availability,
    required double? gapMeters,
    required GeoPoint? sweeperPosition,
    required DateTime now,
  }) {
    if (availability != SweeperAvailability.tracking || gapMeters == null) {
      _samples.clear();
      return SweeperGapTrend.unknown;
    }
    _samples.add(
      _GapSample(
        at: now,
    ...
    if (_samples.length < minimumSamples) return SweeperGapTrend.unknown;
    ...
    if (elapsed.inMilliseconds <= 0) return SweeperGapTrend.unknown;

    if (_sweeperHasStopped()) return SweeperGapTrend.stopped;

    final rate = _gapRateMetersPerSecond();
    if (rate <= -minimumRateMetersPerSecond) return SweeperGapTrend.closing;
    if (rate >= minimumRateMetersPerSecond) return SweeperGapTrend.opening;
    return SweeperGapTrend.holding;
  }

  /// Whether the TEC's own positions have barely moved across the window.
  ///
  /// Deliberately about the sailor's positions rather than about the gap: a gap
  /// can hold steady with both sailors doing 60, and that is not stopped.
  bool _sweeperHasStopped() {
    final positions = _samples
        .map((sample) => sample.sweeperPosition)
        .whereType<GeoPoint>()
  ...
  /// The window a surface should state, so the number a sailor reads is
