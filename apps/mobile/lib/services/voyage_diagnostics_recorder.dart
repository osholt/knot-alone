...
import 'voyage_diagnostics_configuration.dart';
...
/// plain text for the end-of-voyage share. Plain text on purpose: the reader is a
...
///
/// It is deliberately free of Flutter and of the voyage shell, so the pairing
/// logic — the part that answers #412 — can be driven by a synthetic track in a
/// unit test rather than only by riding.
///
/// **Positions in here are the local sailor's own.** Other sailors' positions are
/// someone else's data and are never recorded; see #419.
class VoyageDiagnosticsRecorder {
  VoyageDiagnosticsRecorder({DateTime Function()? clock, this.onEntry})
  ...
  ///
  /// Separate from discarding the recorder because a sailor who switches recording
  /// off mid-voyage has not asked to throw away what was already captured — quite
  ...
  void _add(String line) {
    ...
    // Drop from the front: the end of a voyage is where the sailor was when they
    // noticed something, so the newest entries are the ones worth keeping.
    while (_entries.length > VoyageDiagnosticsConfiguration.maximumEntries) {
      _entries.removeAt(0);
      _dropped += 1;
    }
    onEntry?.call();
  ...
  /// A position fix from the local sailor.
  ///
  /// Kept in a short buffer rather than logged: a fix every second for a
  /// three-hour voyage is ten thousand lines of nothing, and what matters is the
  ...
  /// The app has decided this is the manoeuvre the sailor is riding towards.
  ///
  ...
  /// second derivation here would be subtly different from the one the sailor can
  ...
  void recordManoeuvre({
    ...
    // was silently treated as a repeat, so a whole voyage produced one entry. The
  ...
  /// Emits the comparison for any manoeuvre the sailor has now ridden past.
  ///
  ...
  /// sailor has ridden through the junction.
  ///
  /// "Past" is judged by the sample arriving after the closest approach, not by
  /// distance alone — a sailor the same distance away before and after the junction
  ...
  String render({String? voyageCode, String? appBuild}) {
    final lines = <String>[
      'Tide and Seek · voyage diagnostics',
      if (voyageCode != null) 'Voyage:  $voyageCode',
      if (appBuild != null) 'Build: $appBuild',
      'Written: ${_stamp()}',
      '',
      'Positions in this file are this phone\'s own. No other sailor\'s position,',
      'no voyage or invite secret, and no emergency-contact detail is recorded.',
      '',
      if (_dropped > 0) ...[
        '$_dropped earlier entries were dropped to stay inside the '
            '${VoyageDiagnosticsConfiguration.maximumEntries}-entry bound.',
        '',
      ],
      ..._entries,
    ];
