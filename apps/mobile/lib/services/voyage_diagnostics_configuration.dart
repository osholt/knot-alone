/// Compile-time gate for voyage diagnostics recording.
///
/// The recorder exists because four defects from the 10 August voyage (#412, #409,
/// #418, #414) cannot be settled from a description: each needs what the app said
/// placed beside what the bike then did. Asking a sailor to notice and remember
/// that at 60 mph does not work.
///
/// It writes down where a sailor went, so it is gated the way the test-control
...
/// reaches the phone from outside — the file goes nowhere until the sailor picks a
...
class VoyageDiagnosticsConfiguration {
  ...
  ///
  /// A long voyage must not be able to grow this without limit — the phone is also
  ...
  /// those match the road the sailor was actually on. Measured on the same side of
  /// the junction the engine measures, far enough out that the sailor is on the
