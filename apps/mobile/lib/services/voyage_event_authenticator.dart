...
/// alpha's earlier event body so stored voyages remain readable after upgrade.
class VoyageEventAuthenticator {
  const VoyageEventAuthenticator._();
  ...
  ///
  /// A [VoyageEvent] is immutable, so its verdict under a given secret never
  /// changes - but a voyage journal is re-verified constantly: every reducer
  /// pass, every dashboard build. On a two-hour voyage that is tens of thousands
  /// of events walked tens of thousands of times, which is what made the app
  /// unresponsive at the end of a voyage (#165). Identity keying is what makes
  ...
  /// also releases its entry when the event is collected, so a removed voyage
  /// leaves nothing behind.
  static final Expando<_Verdict> _verdicts = Expando<_Verdict>(
    'voyage event signature verdict',
  );
  ...
  static bool _verifyUncached(VoyageEvent event, String secret) {
  ...
  static String _canonicalV1Body(VoyageEvent event) =>
      _canonicalJson(_v1Map(event));

  static String _transitionalV1Body(VoyageEvent event) =>
      jsonEncode(_v1Map(event));

  static Map<String, Object?> _v1Map(VoyageEvent event) => {
  ...
  static String _legacyBody(VoyageEvent event) => jsonEncode({
    'id': event.id,
    'voyageId': event.voyageId,
    'deviceId': event.deviceId,
    'type': event.type.name,
