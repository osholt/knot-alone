...
/// How a roundabout exit is named to a sailor: left, straight on, right, or back
/// (#427).
///
...
/// right from a right, because a sailor approaching does not either.
///
...
/// changes the word a sailor hears; four means most one-off errors land in the
/// same bucket and change nothing. Collapsing does not fix #412, but it stops
/// most of it reaching the sailor.
///
/// ## Where the boundaries sit, and why slight is straight
///
/// A "slight" exit at a roundabout is one a sailor would call straight on. The
...
enum RoundaboutExitBucket {
  ...
  /// What the sailor hears and reads.
  final String label;
}
...
RoundaboutExitBucket? roundaboutExitBucket(ManeuverDirection direction) =>
    switch (direction) {
      ...
      // 25-35 degrees on a crossing that a sailor would call straight.
...
/// arrow at 47 degrees invites a sailor to read a precision the data does not
...
double roundaboutExitBucketDegrees(RoundaboutExitBucket bucket) =>
    switch (bucket) {
      ...
      // sailor came in on. Offset enough to be seen as its own arm.
