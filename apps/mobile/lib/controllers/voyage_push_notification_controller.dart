...
class VoyagePushNotificationController extends ChangeNotifier
    with WidgetsBindingObserver {
  VoyagePushNotificationController({
  ...
  Future<void> start(VoyageSession session) async {
    if (_closed || _session?.voyageId == session.voyageId) return;
    _session = session;
    ...
    try {
      ...
      if (result.token case final token?) {
        ...
        _statusMessage =
            'Urgent background voyage alerts are enabled. Delivery is best-effort and is not an emergency-service substitute.';
  ...
  Future<void> refreshRegistration() async {
    ...
    try {
      ...
      if (result.token case final token?) {
        ...
        _statusMessage = 'Notification token and voyage role are up to date.';
      } else if (result.permission == PushPermissionState.denied) {
        await _revokeSilently(session);
        _statusMessage =
            'Notifications are blocked by system settings. The voyage registration was removed.';
      } else {
        _statusMessage =
            'A push token is not available yet. Tide and Seek will retry when the app resumes.';
      }
  ...
  Future<void> _revokeSilently(VoyageSession session) async {
    try {
      await registrationApi.revoke(session);
    } on Object {
      // The registration remains voyage-scoped and the server excludes
