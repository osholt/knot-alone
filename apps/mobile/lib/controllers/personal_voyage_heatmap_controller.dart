...
class PersonalVoyageHeatmapCell {
  const PersonalVoyageHeatmapCell({
    required this.x,
    required this.y,
  ...
  GeoPoint get centre {
    const zoom = PersonalVoyageHeatmapBuilder.canonicalZoom;
    final scale = 1 << zoom;
...
/// Derived local coverage. It contains no voyage identity, time, speed or plan.
class PersonalVoyageHeatmap {
  const PersonalVoyageHeatmap({
    required this.cells,
    required this.inputPointCount,
    required this.truncated,
  });

  static const empty = PersonalVoyageHeatmap(
    cells: [],
    inputPointCount: 0,
    truncated: false,
  );

  final List<PersonalVoyageHeatmapCell> cells;
...
class PersonalVoyageHeatmapBuilder {
  const PersonalVoyageHeatmapBuilder({this.maximumCells = 20000});
  ...
  PersonalVoyageHeatmap build(Iterable<CompletedVoyage> voyages) {
    ...
    if (visits.isEmpty) {
      return PersonalVoyageHeatmap(
        cells: const [],
        inputPointCount: inputPointCount,
        truncated: truncated,
      );
    }
    // Absolute rather than normalised-to-this-archive: otherwise one voyage and
    // two identical voyages would both be the archive maximum and render at the
    // same intensity. Eight passages reaches the visual cap.
    final denominator = math.log(9);
    final cells =
        visits.entries
            .map(
              (entry) => PersonalVoyageHeatmapCell(
                x: entry.key.$1,
                y: entry.key.$2,
                visits: entry.value,
...
/// Remembered visibility plus fresh local derivation from [CompletedVoyageStore].
///
/// There is deliberately no HTTP dependency in this boundary. When the store
/// is also a [Listenable] (the production [CompletedVoyagesController] is), a
/// save or deletion rebuilds visible coverage immediately.
class PersonalVoyageHeatmapController extends ChangeNotifier {
  PersonalVoyageHeatmapController._(
    this._store,
    this._preferences,
    this._builder,
    this._visible,
  ...
  static const preferenceKey = 'personal_voyage_heatmap_visible';

  final CompletedVoyageStore _store;
  final SharedPreferences _preferences;
  final PersonalVoyageHeatmapBuilder _builder;
  bool _visible;
  bool _loading = false;
  PersonalVoyageHeatmap _heatmap = PersonalVoyageHeatmap.empty;
  int _refreshGeneration = 0;
  Listenable? _listenableStore;

  static Future<PersonalVoyageHeatmapController> load({
    required CompletedVoyageStore store,
    PersonalVoyageHeatmapBuilder builder = const PersonalVoyageHeatmapBuilder(),
  }) async {
    final preferences = await SharedPreferences.getInstance();
    final controller = PersonalVoyageHeatmapController._(
      store,
      preferences,
      builder,
  ...
  PersonalVoyageHeatmap get heatmap => _heatmap;
  ...
  Future<void> refresh() async {
    ...
    notifyListeners();
    try {
      final voyages = await _store.list();
      final next = _builder.build(voyages);
