...
void main() {
  test('navigation panels preserve map context and sailor clearance', () {
    expect(voyageMapPrimaryPanelFill.toARGB32(), 0xD9252E39);
    expect(
  ...
  test('the app compass replaces the fixed native control', () {
    ...
    expect(source, contains("Key('voyage-compass-position')"));
  });
  ...
  testWidgets(
    'personal voyage heatmap is optional and below the active route',
    (tester) async {
      SharedPreferences.setMockInitialValues({
        PersonalVoyageHeatmapController.preferenceKey: true,
      });
      final archive = InMemoryCompletedVoyageStore();
      ...
      await archive.save(_completedHeatmapVoyage(travelled));
      final heatmap = await PersonalVoyageHeatmapController.load(
        store: archive,
      );
      ...
      await tester.pumpAndSettle();
      expect(find.text('Personal voyages heatmap'), findsOneWidget);
      await tester.tap(
        find.byKey(const Key('personal-voyages-heatmap-toggle')),
      );
      await tester.pumpAndSettle();
  ...
  test('environment map factory can preserve original daytime tiles', () {
    final map = VoyageMapFeature.fromEnvironment(
      restrainedLightMapStyle: false,
    );
  ...
  testWidgets('Android keeps a useful local mini-map while vector tiles load', (
    ...
    final sailors = ValueNotifier<List<MapOverlayMarker>>([
  ...
  testWidgets('pre-start map keeps riding controls and guidance hidden', (
    ...
    for (final key in const [
      ...
      'voyage-compass-position',
    ]) {
      expect(
        find.byKey(Key(key)),
        findsNothing,
        reason: '$key belongs to an active voyage, not the pre-start map',
      );
    }
  });
  ...
  testWidgets(
    'follower waits for the skipper instead of seeing route controls',
    (tester) async {
      final directory = Directory.systemTemp.createTempSync(
  ...
  testWidgets('a started route-less voyage does not block a follower', (
    tester,
  ) async {
    final directory = Directory.systemTemp.createTempSync(
  ...
  testWidgets('a started route-less voyage does not block its skipper', (
    tester,
  ) async {
    final directory = Directory.systemTemp.createTempSync(
      'map-started-skipper-no-route-test',
    );
    addTearDown(() => directory.deleteSync(recursive: true));
    final cache = OfflineTileCache(
      rootDirectory: directory,
  ...
  testWidgets('draws sailor and skipper trails with no route imported', (
    ...
    final trails = ValueNotifier<List<MapOverlayTrace>>([
      const MapOverlayTrace(
        id: 'trail-blake',
        label: 'Blake skipper trail',
        kind: SailorTrailKind.skipper,
        points: [
    ...
    expect(
      ...
      containsAll([
        RouteTrailStyle.skipperTrail.color,
  ...
  testWidgets('no registered TEC hides the distance-to-TEC surface entirely', (
    ...
    final skipperStatus = ValueNotifier<SkipperVoyageStatus?>(
      ...
      const SkipperVoyageStatus(offCourseAlerts: []),
    );
    addTearDown(skipperStatus.dispose);
    final cache = OfflineTileCache(
      rootDirectory: directory,
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: VoyageMapScreen(
          routeStore: InMemoryRouteStore(
            _testRoute(id: 'no-sweeper', name: 'No TEC route'),
          ),
          routeImporter: RouteImporter(source: const _NoFileSource()),
          offlineTileCache: cache,
          skipperStatus: skipperStatus,
    ...
    expect(find.byKey(const Key('skipper-sweeper-gap')), findsNothing);
    expect(find.textContaining('waiting for location'), findsNothing);
    expect(find.textContaining('Sweeper'), findsNothing);

    // Assigning a TEC mid-voyage brings the surface back without a restart.
    skipperStatus.value = const SkipperVoyageStatus(
      sweeperAvailability: SweeperAvailability.stale,
      sweeperName: 'Charlie',
      sweeperLocationAge: Duration(minutes: 4),
      offCourseAlerts: [],
    );
    await tester.pump();
    ...
    skipperStatus.value = const SkipperVoyageStatus(offCourseAlerts: []);
    await tester.pump();

    expect(find.byKey(const Key('skipper-sweeper-gap')), findsNothing);
  });

  testWidgets('the map has no report control outside a voyage', (tester) async {
  ...
  testWidgets('switching to a new voyage store removes a legacy route', (
    tester,
  ) async {
    final directory = Directory.systemTemp.createTempSync(
      'map-new-voyage-store-test',
    );
    addTearDown(() => directory.deleteSync(recursive: true));
    final previousVoyageStore = InMemoryRouteStore(
      _testRoute(id: 'previous', name: 'Previous voyage route'),
    );
    final newVoyageStore = InMemoryRouteStore();
    final cache = OfflineTileCache(
      rootDirectory: directory,
      configuration: const BasemapConfiguration(),
      httpClient: MockClient((_) async => http.Response('', 404)),
    );
    RouteStore activeStore = previousVoyageStore;
    ...
    await tester.pumpAndSettle();
    expect(find.text('Previous voyage route'), findsOneWidget);

    rebuild(() => activeStore = newVoyageStore);
    await tester.pumpAndSettle();
  ...
  testWidgets('forwards the full-screen voyage menu through the app wrapper', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        theme: ThemeData.dark(useMaterial3: true),
        home: VoyageMapFeature(
          routeStore: InMemoryRouteStore(route),
          offlineTileCache: cache,
          mapStyleString:
              '{"version":8,"sources":{},"layers":[{"id":"background","type":"background"}]}',
          navigationPosition: navigation,
          onOpenVoyageMenu: () async => menuOpens += 1,
        ),
      ),
    );
    await tester.pump();
    ...
    expect(find.byType(VoyageMapScreen), findsOneWidget);
    expect(find.byKey(const Key('voyage-menu-button')), findsOneWidget);
    await tester.tap(find.byKey(const Key('voyage-menu-button')));
    await tester.pump();
    expect(menuOpens, 1);
  ...
  testWidgets('aligns portrait ETA with the mini-map header row', (
    ...
    // ActiveVoyageShell owns the moving voyage-menu button (#404), so the map does
    // not receive onOpenVoyageMenu in production. The ETA can therefore align
    ...
    expect(find.byKey(const Key('voyage-menu-button')), findsNothing);
    ...
    (tester) async {
      ...
      final marker = ValueNotifier<MapJunctionMarkerOverlay?>(
        const MapJunctionMarkerOverlay(
          ...
          markerSailorName: 'You',
          isLocalMarker: true,
          sailorsPassed: 2,
          sailorsExpected: 3,
          sweeperDistanceMeters: 210,
          instruction: 'You are holding the junction while sailors pass.',
          stage: MapJunctionMarkerStage.waitingForSailors,
        ),
      );
      ...
      final sailorX = 844 * navigationCameraLandscapeSailorFractionLeftTraffic;
      expect(overlayBounds.left, greaterThan(sailorX + 19));
      ...
      marker.value = const MapJunctionMarkerOverlay(
        markerPoint: GeoPoint(latitude: 53, longitude: -1.01),
        markerSailorName: 'Maya',
        isLocalMarker: false,
        sailorsPassed: 2,
        sailorsExpected: 3,
        sweeperDistanceMeters: 210,
        instruction: 'Maya is holding the junction while sailors pass.',
        stage: MapJunctionMarkerStage.waitingForSailors,
      );
      await tester.pump();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      // The 104 pixel canvas plus the sailor-count caption, which is now in the
      ...
      // Top trailing corner in portrait (#133), opposite the voyage menu, and out
      ...
      expect(
        ...
        reason: 'completed planned route must not be painted behind the sailor',
      );
      // The skipper's trail is the widest line and is drawn under the plan; an
      // off-route trail is dashed and drawn over it.
      final skipper = lineWithColor(RouteTrailStyle.skipperTrail.color);
      expect(skipper.strokeWidth, RouteTrailStyle.skipperTrail.widthPixels);
      expect(
        layer.polylines.indexOf(skipper),
      ...
      // pressed would tell the sailor they were locked in before they were (#141).
      ...
      // Following the sailor is throttled against the wall clock, so a camera
  ...
  testWidgets('shows a stopped-sailor assistance sheet after an alert', (
    tester,
  ) async {
    ...
    // as a fault when nothing at the control says the sailor picks who to text.
    ...
    // #188 dropped the old note's claim that a voyage invite carries no phone
    // numbers: a sailor may now have been given the skipper's or TEC's. Neither
    ...
    expect(
      find.byKey(const Key('emergency-contact-call-sweeper')),
      findsNothing,
    );
  ...
  testWidgets('offers Call and Message where a number was shared, and says so '
    ...
    // #188. The skipper has given this sailor their number; the TEC has not.
    ...
    // number" is information a stopped sailor needs.
      ...
      MaterialApp(
        ...
        home: VoyageMapScreen(
          ...
          onEmergencyContactUsed: (contact) =>
              used.add(contact.contactShareEventId ?? contact.sailorId),
        ),
      ),
    );
    await tester.pump();
    ...
    expect(
      find.byKey(const Key('emergency-contact-message-sweeper')),
      findsNothing,
    );
    expect(
    ...
    // #173's contacts-book fallback survives, because a sailor whose skipper and
    // TEC have shared nothing still needs a way out.
    expect(find.text('Text someone from your contacts'), findsOneWidget);

    // Dialling marks the share used, which is what keeps it past the voyage-end
    // purge.
  ...
  testWidgets('a sailor whose skipper and TEC shared nothing keeps a working '
      'sheet', (tester) async {
  ...
  testWidgets('shows the paused-voyage banner and a working leave button', (
    tester,
  ) async {
    ...
    // Pausing/resuming and ending the voyage are skipper actions that live in
    // the voyage menu (see active_voyage_shell.dart), not on the map itself -
  ...
  testWidgets('the maximum overlay set clears the upper band and itself', (
    ...
    Future<void> verifyLayout({required bool landscape}) async {
      ...
      final rects = <String, Rect>{
        'voyage-paused-banner': tester.getRect(
          find.text('GROUP VOYAGE PAUSED'),
        ),
      };
      ...
      final voyageMenu = rects['voyage-menu-button']!;
      expect(voyageMenu.top, lessThan(size.height / 3));
      expect(voyageMenu.width, lessThanOrEqualTo(48));
      expect(voyageMenu.height, lessThanOrEqualTo(48));
      expect(voyageMenu.left, lessThanOrEqualTo(size.width * 0.1));
      ...
      // "Follow me" is absent unless the sailor has earned it by taking the
      ...
      if (landscape) {
        ...
        for (final key in const [
          ...
          expect(
            rects[key]!.right,
            lessThan(sailorX - 19),
            reason: '$key crosses the sailor anchor in $size',
          );
        }
        for (final key in const ['voyage-compass-position']) {
          expect(
            rects[key]!.left,
            greaterThan(sailorX + 19),
            reason: '$key crosses the sailor anchor in $size',
          );
        }

        // ETA stacks above the group overview in the bottom-left column; the
        // compass stays at top-right.
        ...
        final compass = rects['voyage-compass-position']!;
        expect(compass.right, closeTo(size.width - 10, 1));
        ...
        expect(
          cameraPlan.sailorViewportFraction * size.height + 19,
          lessThanOrEqualTo(guidance.top),
          reason: 'the camera must lift the sailor above the wider card',
        );
        expect(
          rects['skipper-sweeper-gap']!.bottom,
        ...
        final leave = rects['leave-voyage-button']!;
        ...
        // for SOS used to land on LEAVE and drop the sailor out of the voyage.
        final sos = rects['emergency-alert-button']!;
        final leave = rects['leave-voyage-button']!;
        expect(
          sos.bottom,
        ...
        // The compass owns the corner slot it used to share with the speed
        // sign: hard right and clear of the hand reaching for the stack.
        final compass = rects['voyage-compass-position']!;
        expect(compass.right, closeTo(size.width - 12, 1));
        expect(compass.width, closeTo(58, 0.1));
        for (final target in [sos, leave]) {
          expect(
            compass.left,
            greaterThanOrEqualTo(target.right),
            reason: 'the compass must stay clear of the targets in $size',
          );
        }
        ...
        for (final entry in rects.entries) {
          if (entry.key == 'voyage-menu-button' ||
        ...
        expect(voyageMenu.top, closeTo(portraitVoyageMenuTopOffset, 1));
  ...
  Future<void> pumpArrival(
    WidgetTester tester, {
    required ValueNotifier<VoyageCompletionAssessment?> suggestion,
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: VoyageMapScreen(
          ...
          distanceUnit: DistanceUnit.miles,
          voyageCompletionSuggestion: suggestion,
          onEndVoyageForEveryone: onEnd,
          onDismissVoyageCompletion: onDismiss,
          onOpenVoyageMenu: () async {},
          onEmergencyAlert: () async {},
          onLeaveVoyage: () async {},
        ),
  ...
  testWidgets('the arrival question no longer covers the map', (tester) async {
    ...
    // barrier across the whole surface at the one moment a sailor still needs
    // to see where they are going.
    final suggestion = ValueNotifier<VoyageCompletionAssessment?>(arrived);
    ...
    // And the sailor keeps the controls they were using.
    expect(find.byKey(const Key('emergency-alert-button')), findsOneWidget);
  });

  testWidgets('a paused voyage never shows the arrival question', (
    tester,
  ) async {
    // `_maybeAutomaticallyEndVoyage` returns early on a paused voyage, so this can
    // never happen through the shell. Guarded here too, because this is the
    // layer whose band height is measured against the cap.
    final suggestion = ValueNotifier<VoyageCompletionAssessment?>(arrived);
    addTearDown(suggestion.dispose);

    await pumpArrival(tester, suggestion: suggestion, voyagePaused: true);

    expect(find.byKey(const Key('voyage-completion-suggestion')), findsNothing);
  });

  testWidgets('the sailor can wave the arrival question away', (tester) async {
    final suggestion = ValueNotifier<VoyageCompletionAssessment?>(arrived);
    addTearDown(suggestion.dispose);
    var dismissed = 0;
  ...
  testWidgets('ending for everyone still asks first', (tester) async {
    ...
    // confirmation that carries `endVoyageConsequence` - the map only asks.
    final suggestion = ValueNotifier<VoyageCompletionAssessment?>(arrived);
    addTearDown(suggestion.dispose);
    var ended = 0;

    await pumpArrival(tester, suggestion: suggestion, onEnd: () => ended += 1);
    await tester.tap(find.byKey(const Key('confirm-completed-voyage')));
    await tester.pumpAndSettle();
  ...
  testWidgets('the arrival band stays inside the cap with the suggestion up', (
    ...
    // the band with. `_maybeAutomaticallyEndVoyage` returns early on a paused
    // voyage and on an active marker, so the paused banner and the junction card
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: VoyageMapScreen(
          ...
          groupSailorCount: 3,
          distanceUnit: DistanceUnit.miles,
          voyageCompletionSuggestion: ValueNotifier(
          ...
          onEndVoyageForEveryone: () {},
          onDismissVoyageCompletion: () {},
          onOpenVoyageMenu: () async {},
          onEmergencyAlert: () async {},
          onLeaveVoyage: () async {},
        ),
    ...
    // now costs a sailor less than half the screen instead of all of it.
    expect(_bottomChromeFraction(tester, size), lessThan(0.60));
  ...
  testWidgets('the decluttered portrait band lets the camera look ahead', (
    ...
    // turn banner, the action row and the speed sign, no paused voyage, no
    ...
    // which held the sailor at 0.429 of the viewport and aimed the camera 60
    ...
    // sailor, and the marker sits below the centre of the frame where #105 wants
    // it. The band no longer pushes it past the middle.
    expect(plan.forwardBiasPixels, greaterThan(0));
    expect(plan.sailorViewportFraction, greaterThan(0.5));
    ...
    expect(
      projectedViewport!.sailorViewportFraction,
      inInclusiveRange(navigationCameraMinimumSailorFraction, 0.72),
    );
    expect(projectedViewport!.sailorHorizontalViewportFraction, 0.5);
    ...
    for (final previousBand in [431.0, 342.0]) {
      ...
      expect(
        ...
        greaterThan(previous.sailorViewportFraction),
  ...
  testWidgets('a voyage with no route keeps SOS, Leave and the voyage menu', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        home: VoyageMapScreen(
          ...
          onEmergencyAlert: () async => alerts += 1,
          onLeaveVoyage: () async => leaves += 1,
          onOpenVoyageMenu: () async => menus += 1,
        ),
      ),
    );
    await tester.pumpAndSettle();
    ...
    // there, and so is the paused-voyage state.
    expect(find.byKey(const Key('emergency-alert-button')), findsOneWidget);
    ...
    await tester.pump();
    expect(leaves, 1);
    // SOS second, because a stopped sailor's alert opens the assistance sheet
    ...
    expect(find.text('You are stopped'), findsOneWidget);
    Navigator.of(
      tester.element(find.byKey(const Key('leave-voyage-button'))),
    ).pop();
    await tester.pumpAndSettle();

    // A moving fix with no route: the follow camera engages, the riding canvas
    // takes the screen, and the voyage menu appears in its corner. Route-derived
    ...
    // The camera is following a route-less sailor, so let its animation finish
  ...
  testWidgets('a route-less sailor can recover the camera after a pan', (
    tester,
  ) async {
    // #124 plus #125: following works from position and heading alone, and
    // "Follow me" is earned by a manual pan rather than sitting there all voyage.
    ...
    // bike stopping, because that is exactly when the sailor needs it.
    navigation.value = MapNavigationPosition(
  ...
  testWidgets('a stationary sailor who pans away is offered the way back', (
    tester,
  ) async {
    ...
    // pushed off the sailor with no way back. #133 replaced the flag with a
    ...
    // here, and a sailor on a desk had no way to a viewport they had never been in.
    expect(find.byKey(const Key('navigation-follow-button')), findsOneWidget);

    // Tapping goes to the viewport - following the sailor icon at the planned
    // zoom - and only then is the control done.
    await tester.tap(find.text('Follow me'));
    await tester.pumpAndSettle();
    expect(find.byKey(const Key('navigation-follow-button')), findsNothing);

    // One pan is enough. No distance judgement stands between the sailor and the
    ...
    // standing still puts the map back on the sailor.
    navigation.value = MapNavigationPosition(
      point: const GeoPoint(latitude: 53, longitude: -1.015),
      recordedAt: DateTime.utc(2026, 7, 26, 12, 1),
      speedMetersPerSecond: 0,
    ...
    // sat at, measured on an SE, so a map pushed 468 m off the sailor still called
    // itself framed.
  ...
  testWidgets('the configured basemap gives the voyage a tracked camera', (
    tester,
  ) async {
    ...
    // the voyage surface is MapLibre, and MapLibre only reports its camera when
    ...
    // never against the camera the sailor had panned to, so "Follow me" was
    ...
    expect(
      find.byType(ml.MapLibreMap),
      findsOneWidget,
      reason: 'a configured basemap must put the voyage on MapLibre',
    );
  ...
  testWidgets('an action target keeps its bounds through every state', (
    ...
    // exact moment the sailor had just raised an alert. The controls a sailor
    ...
    addTearDown(() => directory.deleteSync(recursive: true));
    // Moving, so the assistance sheet a stopped sailor gets stays out of the way
    // of the measurement.
    final navigation = ValueNotifier<MapNavigationPosition?>(
      MapNavigationPosition(
        point: const GeoPoint(latitude: 53, longitude: -1.015),
    ...
    const actionKeys = ['emergency-alert-button', 'leave-voyage-button'];
    ...
    Future<void> verifyStableThroughStates({required bool landscape}) async {
      ...
      //
      // Both orientations stack SOS over LEAVE. (A third REPORT target sat
      // alongside until the enforcement-sighting surface was removed.)
  ...
  testWidgets('a paused voyage does not move the action targets', (
    tester,
  ) async {
    ...
    await tester.pumpAndSettle();
    final running = {
      for (final key in const ['emergency-alert-button', 'leave-voyage-button'])
    ...
    expect(
      ...
      running,
      reason: 'pausing the voyage moved a target',
    );
  ...
  test('every quick message kind has its own symbol on both phones', () {
    // The kind a sailor chose to send and the kind another sailor is shown are
  ...
  test('where the sender is reads as one of two honest forms', () {
    ...
    // A sailor standing beside you is not "0 m back".
    expect(
      describeQuickMessageOrigin(
        const QuickMessageOrigin(
          distanceMeters: 4,
  ...
  testWidgets('a routine quick message reaches a sailor watching the map', (
    tester,
  ) async {
    // #151: the send path always worked and nothing presented the result. A
    // skipper on the Map tab has to be told who, what, when and where, without
  ...
  testWidgets('an urgent quick message interrupts and leaves the row behind', (
    ...
    // still there, so a sailor who glances away has not lost the alert.
    await tester.tapAt(
    ...
    // Opening the voyage menu rebuilds the map tab. The interrupt must remember
  ...
  testWidgets('the sender is shown that their own alert was seen', (
    ...
    final alerts = ValueNotifier<List<VoyageQuickMessageAlert>>(const []);
    addTearDown(alerts.dispose);
    // Moving, so the assistance sheet a stopped sailor gets after an alert stays
    ...
    // The acknowledgement arrives. The control the sailor already pressed says so,
  ...
  testWidgets('several messages at once are one row and a count', (
    ...
    // Three sailors raising something must not put three rows into the band #125
  ...
  testWidgets('an incoming alert moves nothing and stays out of the road', (
    ...
    // upwards from its bottom anchor, so the targets a sailor reaches for without
    ...
    // the sailor is on, with one manoeuvre ahead of them.
    final route = ImportedRoute(
      id: 'quick-chrome',
    ...
    // Everything at once: a paused voyage, an off-course sailor, the TEC gap, the
    ...
    Future<void> verify({required bool landscape}) async {
      ...
      if (landscape) {
        // The centre column is where the sailor's own marker and the road ahead
...
CompletedVoyage _completedHeatmapVoyage(ImportedRoute traveledRoute) =>
    CompletedVoyage(
      voyageId: 'heatmap-voyage',
      voyageCode: '392725',
...
/// A received quick message as the voyage shell publishes one.
VoyageQuickMessageAlert _quickMessageAlert({
...
/// these tests use - its "route-only offline map" badge adds 36 pixels no sailor
