...
import 'package:tide_and_seek/features/map/voyage_clock.dart';
...
void main() {
  ...
  group('it shows the time and keeps up', () {
    testWidgets('the current time is drawn', (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(body: VoyageClock(clock: () => now)),
        ),
      );

      String shown() =>
          tester.widget<Text>(find.byKey(const Key('voyage-clock'))).data!;
    ...
    testWidgets('nothing is left ticking after it goes', (tester) async {
      ...
      // is built and torn down as the voyage chrome changes.
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: VoyageClock(clock: () => DateTime(2026, 8, 12, 14, 30)),
          ),
        ),
      );
    ...
    testWidgets('ink follows the map palette', (tester) async {
      ...
      expect(
        tester.widget<Text>(find.byKey(const Key('voyage-clock'))).style!.color,
        Colors.black,
      );
  ...
  group('both surfaces draw it themselves', () {
    test('the landscape map places the clock', () {
      // Read as source because the voyage map needs a platform map to build.
      final source = File(
        'lib/features/map/voyage_map_feature.dart',
      ).readAsStringSync();

      expect(source, contains('VoyageClock(darkMap: _basemap.dark)'));
    });
  });
}
