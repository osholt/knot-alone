...
import 'package:tide_and_seek/features/voyage/observer_access_sheet.dart';
...
void main() {
  testWidgets('explains the narrow disclosure and requires explicit consent', (
    ...
    expect(find.textContaining('does not join the voyage'), findsOneWidget);
    expect(
      find.textContaining('does not share the voyage code'),
      findsOneWidget,
    );
  ...
  testWidgets('skipper can create a disclosed whole-group watcher link', (
    tester,
  ) async {
    final api = _FakeObserverApi();
    final controller = ObserverAccessController(api, _MemoryStore());
  ...
  // A sailor reported being unable to leave this screen after sharing a link, at
...
class _MemoryStore implements ObserverGrantStore {
  ...
  @override
  Future<void> delete(String voyageId) async => values = const [];
  ...
  @override
  Future<List<ObserverGrantCredentials>> load(String voyageId) async => values;
