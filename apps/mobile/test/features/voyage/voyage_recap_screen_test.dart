// The recap export's sailor-facing behaviour (#157).
//
...
// basemap still exports, and a route-less voyage does not fail.
//
...
import 'package:tide_and_seek/features/voyage/voyage_recap_screen.dart';
...
void main() {
  ...
  testWidgets('a light app starts the image light', (tester) async {
    ...
    expect(
      tester.widget<VoyageRecapCard>(find.byType(VoyageRecapCard)).dark,
      isFalse,
      reason: 'the first Share should do what the sailor expects',
    );
  });
  ...
  testWidgets('the toggle changes the card and nothing else', (tester) async {
    ...
    final themeBefore = Theme.of(
      tester.element(find.byType(VoyageRecapCard)),
    ).brightness;
    ...
    expect(
      Theme.of(tester.element(find.byType(VoyageRecapCard))).brightness,
      themeBefore,
  ...
  testWidgets('an unconfigured basemap shows no attribution to claim', (
    ...
    final card = tester.widget<VoyageRecapCard>(find.byType(VoyageRecapCard));
  ...
  testWidgets('a route-less voyage still exports', (tester) async {
    // #124: a voyage with no route is valid, and its recap says so.
    await pumpScreen(tester, points: const []);

    expect(find.text('No recorded route for this voyage'), findsOneWidget);
    expect(find.byKey(const Key('share-recap-image-button')), findsOneWidget);
    expect(
      tester.widget<VoyageRecapCard>(find.byType(VoyageRecapCard)).mapLayer,
      isNull,
      reason: 'nothing to frame a basemap around',
    );
  });

  testWidgets('a single-point voyage is treated as route-less', (tester) async {
    await pumpScreen(
      tester,
