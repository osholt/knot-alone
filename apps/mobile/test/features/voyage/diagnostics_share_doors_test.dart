...
/// Every share that hands over a voyage must hand over its recorded log too (#456).
///
...
/// room was left unwired**: `EndedVoyageScreen` called the sharer with no
/// `diagnostics:` argument, and that is the door a sailor uses once the voyage is
/// over. The evidence from a whole voyage was dropped without a word.
///
...
void main() {
  test('every caller of the summary sharer passes the recorded log', () {
    ...
    for (final path in _dartFilesUnder('lib')) {
      ...
      if (!source.contains('VoyageSummarySharer')) continue;
      if (!source.contains('.share(')) continue;
      // The sharer's own definition states the parameter rather than passing it.
      if (path.endsWith('services/voyage_summary_exporter.dart')) continue;
    ...
    expect(
      ...
      isEmpty,
      reason:
          'these call the voyage summary sharer without attaching the recorded '
          'diagnostics log, so a sailor recording a voyage would lose it there',
    );
  });

  test('the ended-voyage screen is one of those callers', () {
    ...
    final source = File(
      'lib/features/voyage/ended_voyage_screen.dart',
    ).readAsStringSync();
  ...
  test('the log can be reached from Settings, not only from a voyage', () {
    // The door that does not depend on being anywhere in particular. A sailor who
    // has finished the voyage and moved on had no way at all before this.
    final source = File(
      'lib/features/settings/voyage_diagnostics_section.dart',
    ).readAsStringSync();
