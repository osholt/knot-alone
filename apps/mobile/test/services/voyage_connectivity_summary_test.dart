...
// green "Server sync succeeded", and an amber "Live sailor positions are paused
// because the voyage service cannot be reached". Every one of them was accurate.
// Together they told a sailor nothing, which is the defect.

import 'package:flutter_test/flutter_test.dart';
import 'package:tide_and_seek/services/voyage_connectivity_summary.dart';

void main() {
  final now = DateTime.utc(2026, 7, 27, 18);

  VoyageConnectivitySummary summarise({
    bool transportActive = true,
    bool positionsPaused = false,
    int queuedEventCount = 0,
    Duration? syncedAgo = Duration.zero,
  }) => VoyageConnectivitySummary.from(
  ...
  test('the reported field state resolves to one answer, and it is no', () {
    ...
    expect(summary.state, VoyageConnectivityState.notReaching);
    expect(summary.headline, 'The group cannot see where you are');
    expect(
      summary.detail,
      contains('resume on their own'),
      reason: 'the sailor needs to know whether to do something',
    );
  });
  ...
  test('everything current and empty reads as reaching', () {
    ...
    expect(summary.state, VoyageConnectivityState.reaching);
  ...
  test('a sync old enough to distrust stops counting as success', () {
    ...
    expect(fresh.state, VoyageConnectivityState.reaching);
    expect(stale.state, VoyageConnectivityState.degraded);
    expect(
      stale.detail,
  ...
  test('never having synced is not success', () {
    ...
    expect(summary.state, VoyageConnectivityState.degraded);
    expect(
      summary.detail,
      contains('Nothing has reached the voyage service yet'),
    );
  });
  ...
  test('a queue is reported with what will happen to it', () {
    expect(
      ...
      '106 voyage events are waiting to send, and will go on the next exchanges.',
    );
    expect(summarise(queuedEventCount: 1).detail, contains('One voyage event'));
    expect(summarise().detail, isNot(contains('waiting to send')));
  });

  test('a bare number never reaches the sailor', () {
