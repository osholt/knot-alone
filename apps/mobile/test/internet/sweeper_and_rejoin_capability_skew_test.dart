...
void main() {
  group('current client, older relay', () {
    test('the two capabilities are advertised and gated by name', () {
      ...
      final advertised = RelayClientDescriptor.current()
          ...
          .split(',');
      expect(advertised, contains('sweeper-role-assignment-v1'));
    ...
    test(
      ...
      () async {
        ...
        await eventStore.append(
          _event(
            id: 'rejoin-share',
            type: VoyageEventType.rejoinRouteShared,
        ...
        expect(
          ...
          containsAll(['sweeper-request', 'sweeper-response', 'rejoin-share']),
        );
        await worker.close();
      },
    );
    ...
    test('a relay that does carry them uploads them', () async {
      ...
      expect(api.uploadedEventIds, ['sweeper-request', 'rejoin-share']);
    ...
    test('the named limitations say what is lost and what still works', () {
      ...
      expect(
        ...
        allOf(
          ...
          contains('skipper will not see it'),
        ),
      );
  ...
  group("a sailor's own number, #188", () {
    test('the capability is advertised and gated by name', () {
      expect(
        RelayProtocolCapabilities.current,
        contains(RelayProtocolCapabilities.sailorContactSharing),
      );
      expect(
        RelayClientDescriptor.current()
            .headers['x-tailendcharlie-capabilities']!
            .split(','),
        contains('sailor-contact-sharing-v1'),
      );
    });
    ...
    test('a relay that cannot carry it withholds the share and names the '
      ...
      final api = _NegotiatedRelayApi(
        capabilities: RelayProtocolCapabilities.current
            .where(
              (capability) =>
                  capability != RelayProtocolCapabilities.sailorContactSharing,
            )
            .toSet(),
      );
      final worker = InternetRelayWorker(
    ...
    test('an older build skips the event whole rather than rejecting the '
        'batch', () {
      // The failure this guards against: `VoyageEvent.fromJson` throws on an
      ...
      // with it.
      expect(
        describeUnsupportedRelayEvent(_rawEvent(type: 'sailorContactShared')),
        isNull,
      );
      expect(
        describeUnsupportedRelayEvent(
          _rawEvent(type: 'sailorContactWithdrawn'),
        ),
        'sailorContactWithdrawn',
      );
    });
  });
  ...
  group('older client, current relay', () {
    ...
    test(
      ...
      () async {
        ...
        final client = MockClient(
          (_) async => http.Response(
            jsonEncode({
              ...
              'events': [
                _rawEvent(id: 'future', type: 'sweeperRoleRevoked'),
                request.toJson(),
              ],
            }),
            200,
        ...
        expect(result.events.map((event) => event.id), ['sweeper-request']);
        expect(result.cursor, 'cursor-1');
        expect(result.ignoredEventCount, 1);
        expect(result.ignoredEventTypes, {'sweeperRoleRevoked'});
        api.close();
      },
    );
    ...
    test(
      ...
      () {
        ...
        final decoded = protocol.decode(
          ...
          expectedVoyageId: _session.voyageId,
...
VoyageEvent _signedEvent({required String id, required VoyageEventType type}) =>
    _sign(
      VoyageEvent(
        id: id,
        voyageId: _session.voyageId,
...
VoyageEvent _sign(VoyageEvent event) => VoyageEvent(
  id: event.id,
  ...
  signature: VoyageEventAuthenticator.sign(event, _session.inviteSecret),
);
